...
other deployment. Prefer the CIMD public lane with mandatory outer and
provider S256 PKCE.

The container starts `mcp/http-server.mjs` on `127.0.0.1:3001` and points Secure MCP Tunnel at that private transport URL. Set `HARPOON_ALLOW_PLAINTEXT_HTTP=true` only in this same-container layout so tunnel-client's constrained OAuth-discovery channel can fetch protected-resource metadata from that loopback listener; authorization-server metadata and every public URL remain trusted HTTPS. The optional compatibility service listens separately on container port `3010`, mapped only to NAS loopback. In public mode it substitutes its own provider callback/state, forces Microsoft's callback to bounded query mode, rejects callback POST bodies, maps bounded provider codes to one-time facade codes, verifies the mandatory outer S256 pair, independently applies S256 to the Microsoft provider leg, and rotates encrypted one-time refresh handles; direct provider codes and upstream refresh tokens never reach ChatGPT. Outer state is capped at 512 UTF-8 bytes and provider codes at 3 KiB. The shared encrypted file enforces independent 320 KiB refresh and 640 KiB ephemeral partitions within its 1 MiB total cap, so forged authorization traffic cannot consume the space reserved for refresh rotation. Compose rotates `json-file` logs at 10 MiB with three files; successful health diagnostics are suppressed and repeated 429 diagnostics are sampled once per minute. The HTTP MCP server independently validates the resulting bearer token's issuer, API audience, delegated scope, and authorized client application, then uses the validated tenant for OBO Graph exchange.

ChatGPT Work rewrites the public tunnel resource identifier to an
OpenAI-internal tunnel gateway identifier in its OAuth request. Configure that
single observed value with
`ONEDRIVE_OAUTH_COMPAT_PROTECTED_RESOURCE_ALIASES`; the compatibility facade
accepts it only when its tunnel ID exactly matches the canonical
`https://api.openai.com/v1/mcp/tunnel_...` resource. Protected-resource metadata
remains canonical. The exact validated ChatGPT resource propagates to the
access-token audience, so configure the same value in
`ONEDRIVE_MCP_PROTECTED_RESOURCE_ALIASES`. Arbitrary alias hosts and other
tunnel IDs are rejected.
