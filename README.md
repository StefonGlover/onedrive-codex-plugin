...
Release `0.6.4+codex.20260814225653` presents each validated OneDrive or SharePoint file URL with only the resolved filename as visible hyperlink text, while preserving the canonical URL and stable item identity in structured results. It retains direct link opening, the 19-tool ChatGPT surface, the full 84-tool trusted-local profile, account-and-drive-isolated state, bounded Office inspection and review, materialized downloads and previews, guarded version restore, enterprise exact-drive reads, and the prior nested-folder performance improvements.
...
node scripts/install-versioned-cache.mjs --confirmed --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260814225653"
```
...
```bash
node scripts/install-versioned-cache.mjs --sync-evidence --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260814225653"
node scripts/install-versioned-cache.mjs --sync-evidence --confirmed --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260814225653"
```
