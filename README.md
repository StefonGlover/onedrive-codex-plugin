...
The focused ChatGPT profile advertises 21 tools. Route descriptive or partial-name discovery through standard `search`, exact filenames/paths/observed OneDrive links through `onedrive_open_files`, broader multi-part reads through `onedrive_read_actions`, and selected readable results through `fetch`. Repeated identical searches and root listings may use a 60-second auth-scoped snapshot; every successful mutation invalidates both caches. For `onedrive_open_files`, pass exactly one of `names` or `urls`, with at most five values, and present each returned `displayLink` so only the resolved filename is visible as hyperlink text rather than the bare URL. Use `onedrive_create_office_file` to build, validate, preview, upload, and read back bounded structured `.docx`, `.xlsx`, or `.pptx` content. Use `onedrive_office_inspect` for bounded structure, `onedrive_office_capabilities` for one exact edit-operation schema and valid example, `onedrive_office_review` for Word comments or Excel notes at exact evidence coordinates, and `onedrive_office_batch_transform` for other structured edits. Use `onedrive_download_file` for a materialized original/PDF and `onedrive_render_preview` for visual QA. Rename/move/copy/share/revoke/version-restore changes go through `onedrive_preview_actions`, then `onedrive_commit_actions` after approval. Direct folder creation, upload, Office creation, export, text write/patch, invite, recycle-bin delete, and recycle-bin restore requests use their named focused tools.
...
- Microsoft Graph drive-item APIs do not expose a native OneDrive comment-thread method. The review tool therefore supports evidence-anchored legacy Word comments and Excel notes only, refuses modern/threaded state it cannot preserve, and does not claim reply, resolution, or PowerPoint-comment parity.
...
The guards preserve the full 84-tool contract for Codex, limit ChatGPT to the reviewed 21-tool allowlist, and reject direct calls to hidden full-profile handlers. They verify the standard search/fetch contract, exact composite-operation enums, direct Office-creation safety fields, targeted edit capability schemas/examples, impact annotations, the 38 KiB OAuth `tools/list` budget, bounded results, and every focused skill's declared tool references. The golden-prompt gate covers every focused tool and checks the metadata cues that distinguish commonly confused actions. The ChatGPT server version includes a deterministic contract hash so ChatGPT invalidates stale tool metadata whenever the advertised surface changes.
...
Run the focused ChatGPT Work beta against the connected account after the mocked suite. This is read-only: it starts the exact 21-tool Work profile, checks standard search plus auth-scoped repeat caching, measures ranked search quality for representative metadata and exact-filename queries, fetches a known document, writes an evidence report, and makes no remote mutations:
