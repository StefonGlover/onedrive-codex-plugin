...
Release `0.6.4+codex.20260814215809` adds direct, validated OneDrive and SharePoint link opening to the focused exact-file action while preserving the 19-tool ChatGPT surface and full 84-tool trusted-local profile. It also retains account-and-drive-isolated state, bounded Office inspection and review, materialized downloads and previews, guarded version restore, enterprise exact-drive reads, and the prior nested-folder performance improvements.
...
node scripts/install-versioned-cache.mjs --confirmed --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260814215809"
```
...
```bash
node scripts/install-versioned-cache.mjs --sync-evidence --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260814215809"
node scripts/install-versioned-cache.mjs --sync-evidence --confirmed --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260814215809"
```
