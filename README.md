...
Release `0.6.4+codex.20260813025943` expands the focused ChatGPT Work surface to 19 tools with account-and-drive-isolated state, bounded Office inspection and review, exact operation schemas, materialized downloads and page previews, version history and guarded restore, and enterprise drive discovery with exact-drive fetches. The release also splits Word, Excel, PowerPoint, and review guidance into focused skills while preserving the full 84-tool profile.
...
node scripts/install-versioned-cache.mjs --confirmed --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260813025943"
```
...
```bash
node scripts/install-versioned-cache.mjs --sync-evidence --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260813025943"
node scripts/install-versioned-cache.mjs --sync-evidence --confirmed --target="$HOME/.codex/plugins/cache/personal/onedrive/0.6.4+codex.20260813025943"
```
