...
- `onedrive_create_sharing_link` changes access to a file or folder. It defaults to dry-run and includes permission-preview data by default; only set `dryRun: false` and `confirmed: true` with `expectedName` or `expectedId` after explicit user authorization for the exact item and link scope.
